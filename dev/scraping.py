...
def get_provinces() -> tuple[Province, ...]:
    ...
    provinces = tuple(Province(int(c), n, v) for c, n, v in zip(codes, names, levels))
    return provinces


def get_wards(index: int, province_code: int) -> tuple[Ward, ...]:
    ...
    if sep == '':
        logger.warning('Unrecognized data')
    ...
    for row in rows:
        ...
        wards.append(Ward(int(info[0]), info[1], info[3], province_code))
    return tuple(wards)


def scrape_danhmuchanhchinh():
